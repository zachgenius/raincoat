...
- [x] GoogleTest wiring; every module test registered with `ctest` — *CMakeLists.txt*
...
- [x] `raincoat init [--force|-f]` sets `init_force` — *cli / test_cli* (consolidation)
- [x] Repeatable flags accumulate; everything after `--` is verbatim command — *cli / test_cli*
- [ ] `main` dispatch wired to real behavior for every subcommand — *main* (partial; finalized in Integration)

### 1.3 Default (non-strict) behavior  (runner)
- [ ] Create temp sandbox dir — *runner* (Integration)
- [ ] Fake HOME `<temp>/home/user`, plus `<temp>/tmp`, `<temp>/out` — *runner*
- [ ] Set `HOME`, `TMPDIR`, `TZ=UTC`, `LANG=en_US.UTF-8`, `LC_ALL=en_US.UTF-8` — *runner + env_guard*
- [x] Clear most inherited env; preserve safe base (`PATH`, `TERM`), generic `USER=user` — *env_guard / test_env_guard*
- [ ] Real HOME never mounted (verified end-to-end) — *runner + fs_guard* (unit done; e2e in Integration)
- [x] cwd mounted RW by default in non-strict — *fs_guard / test_fs_guard*
- [x] **Guard: do NOT auto-mount cwd when cwd == $HOME** (privacy) — *fs_guard / test_fs_guard* (consolidation)
- [ ] Run target command through `bwrap` (fork/exec, exit-code propagation) — *runner* (Integration)

### 1.4 Strict mode  (--strict)
- [x] Deny cwd unless explicitly allowed — *fs_guard / test_fs_guard*
- [x] Network OFF by default in strict — *runner resolve_config* (unit in test_runner; e2e Integration)
- [x] Scrub all non-essential env — *env_guard*
- [x] Generic locale/timezone — *env_guard / runner*
- [ ] Warn when command may not work (no writable paths allowed) — *runner + audit* (Integration)
...
- [ ] End-to-end: sandboxed process cannot see real `~/.ssh` etc. — *Integration test*
...
- [x] Log allowed/set/scrubbed by NAME only, never values — *audit / test_audit*

### 1.7 Locale & timezone
- [x] Defaults `TZ=UTC`, `LANG`/`LC_ALL=en_US.UTF-8` — *env_guard / runner*
- [-] Minimal `/etc` view (hostname/hosts/resolv.conf/localtime) — *bwrap* (best-effort; resolv.conf bind on net full done; rest minimal per SPEC "don't overcomplicate")
...
- [ ] Wired into runner env + audit — *runner* (Integration)
...
- [ ] End-to-end: `--net off` actually blocks network — *Integration test*

### 1.10 Audit log  (audit, report, runner)
- [x] Human-friendly start block (command, mode, network, fake HOME, workdir, allowed r/w, env allowed/set/scrubbed, tz, locale, fontconfig, bwrap command) — *audit / test_audit*
- [x] **Structural secret redaction** of `--setenv` values (argv-level, not lossy string) — *bwrap::redact_argv_for_audit + audit / tests* (consolidation)
- [x] `write_audit` append + mkdir -p parent — *audit / test_audit*
- [x] Exit-code end line — *audit / test_audit*
- [ ] Default path `.raincoat/audit.log` (or temp) + `--audit-log` override, written at runtime — *runner* (Integration)
- [x] `raincoat report` summarizes latest audit (playful-but-pro) — *report / test_report* (dispatch wired in Integration)
...
- [x] `load_profile` maps all fields via the toml module — *profile / test_profile* (consolidation: delegates to toml)
- [x] Reject wrong-typed `strict`/`network`/`fontconfig.enabled` (no silent privacy downgrade) — *profile / test_profile* (consolidation)
...
- [ ] Profile actually loaded + merged in the live run — *runner resolve_config* (Integration)

### 1.12 init  (init)
- [x] `default_toml()` matches SPEC schema — *init / test_init*
- [x] `write_init` refuses overwrite unless `--force` — *init / test_init* (consolidation)
- [ ] `raincoat init` creates `.raincoat.toml` end-to-end — *main* (Integration)
...
- [ ] `raincoat doctor` end-to-end exits 0 here / nonzero when unusable — *main* (Integration)
...
- [ ] Missing bwrap message (multi-line, with install hints) surfaced at run time — *runner / main* (Integration)

### 1.15 Tests (SPEC's 10 required)
- [x] 1 CLI parsing · [x] 2 Profile loading · [x] 3 Env scrubbing · [x] 4 --allow-env · [x] 5 --set-env override
- [x] 6 Strict ⇒ net off (unit) · [x] 7 Normal ⇒ cwd access (unit) · [x] 8 Audit log creation (unit)
- [x] 9 Missing command error · [x] 10 Missing bwrap handling (unit)
- [ ] Integration tests invoking real `bwrap` (skip gracefully if absent) — *test_integration* (Integration)

### 1.16 Docs
- [x] SPEC / DESIGN / EGRESS / PROGRESS docs
- [ ] README: what it is, problem, usage, protects / does-not-protect, Linux-first, bwrap dep, limitations, example profiles, roadmap — *README.md* (Docs phase)
- [ ] Honest positioning + warning text present — *README.md*

### 1.17 Verification gate (Phase 1 done when ALL true)
- [ ] `cmake && cmake --build` clean under `-Wall -Wextra`
- [ ] `ctest` 100% green (all module + integration tests)
- [ ] Real runs work: `raincoat -- env` (secrets hidden, fake HOME), `raincoat --net off -- ...`, `raincoat --strict -- ...`, `raincoat doctor`, `raincoat init`, `raincoat report`
